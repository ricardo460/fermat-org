 ...
 * @return {[type]}            [description]
 */
exports.updPlatfrmsDeps = function(old_name, new_name, callback) {
	'use strict';
	var upd = false;
	try {
		platfrmMod.getPlatfrms(function(err, platfrms) {
			...
			if (platfrms) {
				var loopUpdDeps = function(i) {
					if (i < platfrms.length) {
						upd = false;
						for (var j = 0; j < platfrms[i].deps.length; j++) {
							if (platfrms[i].deps[j] === old_name) {
								platfrms[i].deps[j] = new_name;
								upd = true;
							}
						}
						if (upd)
							platfrmMod.updateDepsPlatfrmById(platfrms[i]._id, platfrms[i].deps[j],
								function(err, res_upd) {
									if (err) return callback(err, null);
									if (res_upd) loopUpdDeps(i++);
									else return callback("Error updating deps", null);
								});
					} else return callback(null, true);
				};
				loopUpdDeps(0);
			} else return callback("Platforms no found", null);
		})
	} catch (err) {
...
/**
 * [updSuprlayDeps description]
 * @param  {[type]}   old_name [description]
 * @param  {[type]}   new_name [description]
 * @param  {Function} callback [description]
 * @return {[type]}            [description]
 */
exports.updSuprlayDeps = function(old_name, new_name, callback) {
	'use strict';
	var upd = false;
	try {
		suprlayMod.getSuprlays(function(err, suprlays) {
			...
			if (suprlays) {
				var loopUpdDeps = function(i) {
					if (i < suprlays.length) {
						...
						for (var j = 0; j < suprlays[i].deps.length; j++) {
							if (suprlays[i].deps[j] === old_name) {
								suprlays[i].deps[j] = new_name;
								upd = true;
							}
						}
						if (upd)
							suprlays.updateDepsSuperlayById(suprlays[i]._id, suprlays[i].deps[j],
								function(err, res_upd) {
									if (err) return callback(err, null);
									if (res_upd) loopUpdDeps(i++);
									else return callback("Error updating deps", null);
				...
				loopUpdDeps(0);
			} else return callback("Superlays no found", null);
		})
	} catch (err) {
		return callback(err, null);
	}
