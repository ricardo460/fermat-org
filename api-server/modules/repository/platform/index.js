...
exports.insOrUpdPlatfrm = function(code, name, logo, deps, platfrm_index, layer_index, callback) {
    ...
    platfrmSrv.findPlatfrmByCode(code.toUpperCase(), function(err_plat, res_plat) {
        //console.dir(err_plat);
        //console.dir(res_plat);
        if (err_plat) {
            console.log('step 1')
            callback(err_plat, null);
        } else if (res_plat) {
            console.log('step 2')
            //TODO: update
            var set_obj = {};
            ...
            if (Object.keys(set_obj).length > 0) {
                platfrmSrv.updatePlatfrmById(res_plat._id, set_obj, function(err_upd, res_upd) {
                    if (err_upd) callback(err_upd, null);
                    else callback(null, res_plat);
                });
            } else {
                callback(null, res_plat);
            }
        } else {
            console.log('step 3')
            //TODO: insert
            ...
            platfrmSrv.insertPlatfrm(platfrm, function(err_ins, res_ins) {
                if (err_ins) callback(err_ins, null);
                else callback(null, res_ins);
            });
        }
    })
};